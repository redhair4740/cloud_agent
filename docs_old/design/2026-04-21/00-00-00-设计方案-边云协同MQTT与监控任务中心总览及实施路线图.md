...
V1 默认心跳周期为 `20` 秒，边缘端也可在心跳 payload 中通过 `heartbeat_interval_sec` 或 `heartbeatIntervalSec` 上报实际周期；未上报时使用 Cloud 配置 `vmesh.edge.runtime.heartbeat-interval-sec`。
...
- 节点异常离线：
  收到 `client.disconnected` 或超过 `max(vmesh.edge.runtime.heartbeat-timeout-sec, heartbeatIntervalSec * 3)` 未收到心跳；当前实现由 `EdgeRuntimeServiceImpl#scanHeartbeatTimeouts` 定时巡检并打开 `OFFLINE` 告警，后续心跳恢复后关闭该告警。
- 高延迟：
...
- 每天一个 Bitmap
- 每个槽位表示一个心跳周期
- 单日槽位数按实际 `heartbeatIntervalSec` 动态计算；默认 20 秒时为 `4320`
- TTL 建议 `48` 小时
