...
| 角色 | 文件 | 规则继承 | 适用场景 |
|------|------|----------|----------|
| Fullstack Agent | `10-fullstack-linkage-agent.md` | 全部规则 + 联动规则 | 前后端联动任务 |
| Backend Agent | `20-backend-agent.md` | 基线 + 后端规则 | 后端单侧闭环任务 |
...
项目目录映射与技术栈参数的唯一配置来源，AI 代理和投影脚本从此文件读取变量值：
...
## model-adapters.md — 跨模型适配层

定义不同 AI 模型（GPT / Claude / Gemini 系列）的能力分类与降级策略：

- **上下文窗口**：大（≥128K）/ 中（32K-128K）/ 小（<32K），决定规则加载范围
- **指令格式**：高级（XML 标签）/ 中级（Markdown 结构）/ 基础（纯文本列表），决定指令格式降级
- **工具调用**：原生支持 / 部分支持 / 不支持，决定操作执行方式

降级路径为单向不可逆：`XML 标签 → Markdown 结构 → 纯文本列表`
