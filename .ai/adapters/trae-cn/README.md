...
| 源 | 投影目标 | 投影方式 |
|----|----------|----------|
| `AGENTS.md` § 1 读取顺序 | `project_rules.md` § 上下文加载 | 转写为 Trae-CN 可理解的规则引用说明 |
...
- Trae-CN 原生支持 `.trae/rules/` 目录自动加载，因此规则文件可直接按原文复制，无需内联合并。
- `.ai/agents/` 和 `.ai/skills/` 的内容不默认投影到 `.trae/rules/`，按需在 `project_rules.md` 中引用路径即可。
