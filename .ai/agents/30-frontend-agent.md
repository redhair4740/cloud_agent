...
用于前端单侧闭环任务。API SDK、Pinia、Mock、原型参考和验证要求以 `./.ai/rules/20-frontend-development-rules.md` 为准。
...
4. `./.ai/rules/20-frontend-development-rules.md`

## 3. 开发前判断

- 先用 `./.ai/skills/task-classifier/SKILL.md` 判断 `api_first_required`。
- 确认当前需求能否在既有 OpenAPI 契约下完成。
- 确认接口状态是否允许真实联调或 Mock，状态读取 `./.ai/api-status.yml`。
