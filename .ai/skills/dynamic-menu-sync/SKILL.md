...
- 前端静态路由：`{{dirs.frontend}}/src/router/modules/remaining.ts`
- 前端动态菜单：`{{dirs.frontend}}/src/store/modules/user.ts`
- 前端路由生成：`{{dirs.frontend}}/src/store/modules/permission.ts`
- 路由组件匹配：`{{dirs.frontend}}/src/utils/routerHelper.ts`
- 后端登录菜单接口：`{{dirs.backend}}/vmesh-module-system/src/main/java/**/AuthController.java`
- 后端菜单转换：`{{dirs.backend}}/vmesh-module-system/src/main/java/**/AuthConvert.java`
- 菜单表结构：`{{dirs.backend}}/sql/postgresql/ruoyi-vue-pro.sql`
...
```bash
rg -n "views/<module>|path: '/<module>'|<ComponentName>" {{dirs.frontend}}/src/router/modules/remaining.ts
```
...
```bash
rg -n "@PreAuthorize|hasPermission\('" {{dirs.backend}}/vmesh-module-*/src/main/java
```
...
```text
{{dirs.backend}}/{{dirs.sql}}/yyyy-MM-dd/yyyy-MM-dd-HH-mm-ss.sql
```
...
```bash
rg -n "views/<module>|path: '/<module>'|<ComponentName>" {{dirs.frontend}}/src/router/modules/remaining.ts
rg -n "<菜单名>|<component>|<permission>" {{dirs.backend}}/{{dirs.sql}}/yyyy-MM-dd/*.sql
```

建议确认后再执行：

- 前端类型检查：`cd {{dirs.frontend}} && npm run ts:check`
