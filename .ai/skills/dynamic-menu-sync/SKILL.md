...
本 skill 用于处理前后端菜单、路由、按钮权限的同步问题。默认按 fullstack 联动任务处理，并继承项目 rules，不替代 `.ai/rules`。
...
- 前端静态路由：`WF_VMesh_Coud_UI/src/router/modules/remaining.ts`
- 前端动态菜单：`WF_VMesh_Coud_UI/src/store/modules/user.ts`
- 前端路由生成：`WF_VMesh_Coud_UI/src/store/modules/permission.ts`
- 路由组件匹配：`WF_VMesh_Coud_UI/src/utils/routerHelper.ts`
- 后端登录菜单接口：`WF_VMesh_Coud/vmesh-module-system/src/main/java/**/AuthController.java`
- 后端菜单转换：`WF_VMesh_Coud/vmesh-module-system/src/main/java/**/AuthConvert.java`
- 菜单表结构：`WF_VMesh_Coud/sql/postgresql/ruoyi-vue-pro.sql`
...
```bash
rg -n "views/<module>|path: '/<module>'|<ComponentName>" WF_VMesh_Coud_UI/src/router/modules/remaining.ts
```
...
```bash
rg -n "@PreAuthorize|hasPermission\('" WF_VMesh_Coud/vmesh-module-*/src/main/java
```
...
```text
WF_VMesh_Coud/sql/postgresql/yyyy-MM-dd/yyyy-MM-dd-HH-mm-ss.sql
```
...
```bash
rg -n "views/<module>|path: '/<module>'|<ComponentName>" WF_VMesh_Coud_UI/src/router/modules/remaining.ts
rg -n "<菜单名>|<component>|<permission>" WF_VMesh_Coud/sql/postgresql/yyyy-MM-dd/*.sql
```

建议确认后再执行：

- 前端类型检查：`cd WF_VMesh_Coud_UI && npm run ts:check`
