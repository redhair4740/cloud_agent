...
- SQL 目录读取 `.ai/project.yml` 的 `dirs.backend` 与 `dirs.sql`，落点为 `<backend-dir>/<sql-dir>/yyyy-MM-dd/yyyy-MM-dd-HH-mm-ss.sql`。
...
- 涉及 `<backend-dir>/<edge-module>` 时，优先读取 `.ai/project.yml` 的 `dirs.backend` 与 `backend_stack.edge_module` 后执行：`cd "$BACKEND_DIR" && mvn compile -pl "$EDGE_MODULE" -am`。
