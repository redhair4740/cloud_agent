...
- 新增数据库脚本统一放在 `<backend-dir>/<sql-dir>/` 下；`<backend-dir>` 与 `<sql-dir>` 分别读取 `.ai/project.yml` 的 `dirs.backend` 与 `dirs.sql`，当前项目数据库以 PostgreSQL 为准，新增变更只提交 PostgreSQL 脚本，不再额外补 MySQL 或其他数据库脚本。
- 增量 SQL 必须放到当天日期目录，目录格式为 `yyyy-MM-dd`；若目录不存在，先创建对应日期目录再新增脚本。
- 单个 SQL 文件命名必须为 `yyyy-MM-dd-HH-mm-ss.sql`，例如 `<backend-dir>/<sql-dir>/2026-04-20/2026-04-20-14-30-45.sql`，用秒级时间戳降低多人并行提交时的命名冲突概率。
